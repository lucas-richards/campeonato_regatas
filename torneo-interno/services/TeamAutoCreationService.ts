 ...
 */
export const autoCreateTeams = (players: TeamPlayer[]): TeamPlayer[][] => {
  // First, calculate the average level of all players.
  const n = players.length;
  let averageLevel = 0;
  for (const player of players) {
    averageLevel += player.level?.value || 0;
  }
  averageLevel /= n;
  ...
  for (const player of players) {
    result[player.position?.value || 0 - 1].push(player);
  }

  // Sort the groups by their average level.
  result.sort((group1, group2) => {
    const averageLevel1 =
      group1.reduce((sum, player) => sum + player.level?.value || 0, 0) /
      group1.length;
    const averageLevel2 =
      group2.reduce((sum, player) => sum + player.level?.value || 0, 0) /
      group2.length;
